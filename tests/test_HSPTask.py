...
class TestHSPTask(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open(f'{cls.taskname}.par', 'w') as fp:
            fp.write(wTxt)
    ...
    def test__init_HSPTask__dict_param_by_hand(self):
        hsp = heasoftpy.HSPTask(self.taskname)
        hsp.infile = 'INFILE'
        hsp.number = 7
    ...
    def test__init_HSPTask__dict_wrong_type(self):
        hsp = heasoftpy.HSPTask(self.taskname)
        with self.assertRaises(ValueError):
    ...
    # logfile is a parameter of the task (in addition to being general
    # heasoftpy parameter)
